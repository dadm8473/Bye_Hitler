...
void CMap::Render(LPD3DXSPRITE sprite)
{
	...
	{
		for (int i = 0; i < 2; i++)
			land[i].Draw(sprite, gAssetManager->land1Texture);
	}
	if (stage == 1)
	{
		for (int i = 0; i < 2; i++)
			land[i].Draw(sprite, gAssetManager->land2Texture);
	}
	for (int i = 0; i < 2; i++)
		cloud1[i].Draw(sprite, gAssetManager->cloud1Texture);
	for (int i = 0; i < 2; i++)
		cloud2[i].Draw(sprite, gAssetManager->cloud2Texture);
}